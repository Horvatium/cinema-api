...
router.post('/', auth, async (req, res) => {
    ...
    const { title, genre, duration_minutes, age_rating, synopsis, director, release_year, poster_url } = req.body;

    if (!title || !genre || !duration_minutes || !age_rating) {
        return res.status(400).json({ message: 'Naslov, žanr, dolžina filma in starostna ocena so potrebni.' });
    }

    try {
        const [result] = await db.query(
            'INSERT INTO films (title, genre, duration_minutes, age_rating, synopsis, director, release_year, poster_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    try {
        const [result] = await db.query(
            `UPDATE films SET 
             ...
             WHERE id = ?`,
            [title, genre, duration_minutes, age_rating, synopsis, director, release_year, poster_url, req.params.id]
        );
